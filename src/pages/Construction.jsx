  ...
  return (
    <>
      <section className="container-padded pt-36">
        ...
        <div className="mt-24 mb-16">
          ...
          >
            <h2 className="section-title mb-4">
              Your Journey with ZamungClient
            </h2>
            <motion.div
              initial={{ width: 0 }}
        ...
        <motion.button
          onClick={() => setShowRestaurantPopup(true)}
          className="fixed bottom-8 right-8 z-50 w-16 h-16 bg-gradient-to-r from-accent to-orange-500 rounded-full shadow-2xl flex items-center justify-center group hover:shadow-3xl transition-all duration-300"
        ...
        >
          ...
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 px-3 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-lg">
            Restaurant & Cafe Services
            <div className="absolute top-1/2 left-full -translate-y-1/2 w-0 h-0 border-t-4 border-b-4 border-l-4 border-t-transparent border-b-transparent border-l-slate-900"></div>
          </div>
        </motion.button>
      </section>
        ...
        {showRestaurantPopup && (
          ...
          >
            ...
            >
              <motion.button
                onClick={() => setShowRestaurantPopup(false)}
                className="absolute top-6 right-6 z-50 w-10 h-10 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 group"
                whileHover={{ scale: 1.1, rotate: 90 }}
                whileTap={{ scale: 0.9 }}
              >
                <X className="w-5 h-5 text-slate-700 group-hover:text-accent transition-colors duration-300" />
              </motion.button>

              <div className="max-h-[90vh] overflow-y-auto">
                <RestaurantCafeSection />
              </div>
            </motion.div>
          </motion.div>
        )}
